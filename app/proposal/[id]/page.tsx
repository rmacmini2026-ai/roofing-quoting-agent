            ...
            <tbody>
              {costs.lineItems.map((item) => (
                <tr key={item.name} className="border-b border-slate-50">
                  ...
                  <td className="py-2.5 text-right text-slate-500">{item.unit}</td>
                  <td className="py-2.5 text-right text-slate-600">${item.unitPrice.toFixed(2)}</td>
                  <td className="py-2.5 text-right font-medium text-slate-800">${item.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
              <tr className="border-b border-slate-100">
                <td colSpan={4} className="py-2.5 text-slate-500 italic">Labor (installation)</td>
                <td className="py-2.5 text-right font-medium text-slate-800">${costs.laborCost.toLocaleString()}</td>
              </tr>
            </tbody>
            <tfoot>
              <tr>
                <td colSpan={4} className="pt-3 text-slate-500 text-xs">Materials subtotal</td>
                <td className="pt-3 text-right text-slate-600">${costs.materialSubtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
              </tr>
              {costs.taxRate > 0 && (
                <tr>
                  <td colSpan={4} className="py-1 text-slate-500 text-xs">Sales tax ({(costs.taxRate * 100).toFixed(2)}%)</td>
                  <td className="py-1 text-right text-slate-600">${costs.tax.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                </tr>
              )}
              <tr className="border-t-2 border-slate-800">
                <td colSpan={4} className="pt-3 text-base font-bold text-slate-800">Total Project Investment</td>
                <td className="pt-3 text-right text-2xl font-bold text-sky-600">${costs.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
              </tr>
            </tfoot>
          </table>
        </Card>
